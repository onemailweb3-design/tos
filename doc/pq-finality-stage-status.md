...
| Unverified finality admission | `test-pending-finality-cache`, `finality-evidence-admission-source` | Public and fast-sync Plumtree IDs commit to the block and canonical signature set; authenticated sender identity and the exact received boxed-TL byte count reach the manager without reserialization; the manager rejects missing accounting and classifies a remote sender against the exact current/next validator set named by the evidence. A trusted-state-scoped two-entry memo collapses 64 repeated arrivals for one `(shard, cc_seqno, set_hash)` to one set computation; the 400-validator shard-set benchmark measured 390 us and 524,800 copied PQ-key bytes per set on the development host. One sender gets one unverified candidate per block and at most 1 MiB. Non-validator public-overlay peers share 16 MiB; a disjoint 400 MiB pool holds one complete maximum-size committee, so filling the public pool cannot displace valid validator evidence. The 4096-byte minimum charge and the two pools cap retained unverified finality evidence at exactly 436,207,616 bytes (416 MiB). Retry/expiry APIs require callers to pass both `now` and `expires_at` explicitly, so omitting the clock is a compile error. | A live Plumtree peer graph or the actor-scheduled `ValidatorManagerImpl` coroutine. The behavioural gate drives the production transport-ID helper, authority memo/classifier, ingress decision, and pending-store implementation; the source gate pins their composition in the manager and transport ingress. It cannot distinguish a legitimately local manager call from an upstream remote path that incorrectly omits its authenticated peer; that handoff remains source-pinned. Classification uses only a validator set whose hash and catchain sequence match the evidence; evidence naming unavailable historical coordinates uses the public pool until its trusted context is available. The two-entry memo bounds repetition of a coordinate triple, not a flood that continually changes all three coordinates. The 400 MiB pool does not promise two independent full-committee reservations during a validator-set rotation. Locally originated evidence has no received payload and uses reserved capacity charged by intrinsic signature bytes. |
