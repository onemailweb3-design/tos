...
CASES = [
    ...
    Case('message-canonical', 'a digest outside the field is signed anyway', AUTH,
         '  throw_unless(106, (digest >= 0) & (digest < field_modulus()));',
         '  throw_unless(106, digest >= 0);',
         SIGNED_TEST),

    # --- 9.1, two signatures always ---------------------------------------
    Case('slot-0-signature', 'the signature in input slot 0 is not verified', AUTH,
         '  throw_unless(107, verify_intent_signature(public_key_0, signature_0, digest));\n', '',
         SUBSTITUTION_TEST),
    Case('slot-1-signature', 'the signature in input slot 1 is not verified', AUTH,
         '  throw_unless(108, verify_intent_signature(public_key_1, signature_1, digest));\n', '',
         SUBSTITUTION_TEST),
    ...
    Case('validity-uint32', 'valid_until is no longer held to the uint32 field', AUTH,
         '  throw_unless(109, (valid_until >= 0) & (valid_until < uint32_limit()));\n', '',
         VALIDITY_TEST),
    Case('validity-past', 'a valid_until already in the past is accepted', AUTH,
         '  throw_unless(110, valid_until >= now_seconds);\n', '',
         VALIDITY_TEST),
    Case('validity-window', 'the one-hour ceiling on valid_until is removed', AUTH,
         '  throw_unless(111, valid_until <= now_seconds + intent_validity_window());\n', '',
         VALIDITY_TEST),
]
