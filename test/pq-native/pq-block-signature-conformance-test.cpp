...
td::Result<td::Ref<block::BlockSignatureSet>> persisted(
  ...
  return fixture.persisted_final(signatures, session, slot, candidate, weight,
                                 trusted_set->get_validator_set_hash(), trusted_set->get_catchain_seqno(), trusted_set);
}
...
td::Result<tos::ValidatorWeight> verify_final(
  ...
  return signature_set->check_signatures(std::move(trusted_set), block_id);
}
...
int main() {
  ...
  expect_error(verify_final(fixture, tampered, fixture.session, Fixture::slot, base_candidate, quorum_weight, fixture.id,
                            fixture.validator_set),
  ...
  expect_error(persisted(fixture, unknown, fixture.session, Fixture::slot, base_candidate, quorum_weight,
                         fixture.validator_set),
  ...
  expect_error(block::BlockSignatureSet::create_simplex_pq_final(
                   std::move(wrong_algorithm), Fixture::catchain_seqno,
  ...
  auto pq_with_mixed_hash = require_ok(block::BlockSignatureSet::create_simplex_pq_final(
                                           clone_pairs(quorum), Fixture::catchain_seqno,
                                           mixed_set->get_validator_set_hash(), fixture.session, Fixture::slot,
                                           candidate(fixture.id)),
  ...
  auto approve = require_ok(block::BlockSignatureSet::create_simplex_pq_approve(
                                clone_pairs(approve_pairs), Fixture::catchain_seqno,
                                fixture.validator_set->get_validator_set_hash(), fixture.session, Fixture::slot,
                                candidate(fixture.id)),
                            "valid-approve-construction");
  if (require_ok(approve->check_approve_signatures(fixture.validator_set, fixture.id), "valid-approve") !=
      quorum_weight) {
    fail("PQ_BLOCK_SIGNATURE_WEIGHT_MISMATCH case=valid-approve");
  }
  expect_error(approve->check_signatures(fixture.validator_set, fixture.id), "not final signatures",
               "notarize-presented-as-final");

  auto final_in_memory = require_ok(block::BlockSignatureSet::create_simplex_pq_final(
                                        clone_pairs(quorum), Fixture::catchain_seqno,
                                        fixture.validator_set->get_validator_set_hash(), fixture.session, Fixture::slot,
                                        candidate(fixture.id)),
                                    "valid-final-construction");
  expect_error(final_in_memory->check_approve_signatures(fixture.validator_set, fixture.id), "not approve signatures",
               "finalize-presented-as-approve");

  auto duplicate = clone_pairs(quorum);
  duplicate[1].validator_id = duplicate[0].validator_id;
  expect_error(block::BlockSignatureSet::create_simplex_pq_final(
                   std::move(duplicate), Fixture::catchain_seqno, fixture.validator_set->get_validator_set_hash(),
  ...
  std::printf("PQ_BLOCK_SIGNATURE_CONFORMANCE_OK positive_cases=2 negative_cases=21\n");
  return 0;
}
