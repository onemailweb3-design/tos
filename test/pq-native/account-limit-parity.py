...
def compile_probe(build, out):
    ...
    subprocess.run([str(build / 'crypto/fift'), '-I',
                    f'{ROOT}/crypto/fift/lib:{ROOT}/crypto/smartcont', '-s', str(run)],
                   check=True, capture_output=True)
...
def size_limits_v2():
    ...
    from cells import Cell
    return (Cell().uint(0x02, 8)
            .uint(1 << 21, 32).uint(1 << 13, 32).uint(1000, 32).uint(512, 16)
            .uint(65535, 32).uint(512, 16)
            .uint(ORDINARY_CEILING, 32).uint(MASTERCHAIN_CEILING, 32)
            .uint(256, 32).uint(256, 32).uint(2, 32).uint(8, 8).uint(26, 32))


def configuration(global_version):
    from cells import Cell, read_dict, make_dict, from_boc
    import native
...
def report(left, right):
    ...
    if differences:
        raise SystemExit('executors disagree:\n' + '\n'.join(f'  cpp: {a}\n  rust: {b}'
                                                             for a, b in differences))
    return {'cases': len(rows), 'accepted': sum(1 for r in rows if r[2] == '0'),
...
def main():
    ...
    shapes = [('mc-oversized', -1, payload), ('mc-small', -1, small),
              ('basechain-oversized', 0, payload), ('basechain-small', 0, small)]
    ...
    for version in (11, 12, 16):
        ...
        try:
            for shape, workchain, body_payload in shapes:
                ...
                rows.append('\t'.join([
                    name, str(details.get('exit')),
                    str((details.get('action') or {}).get('code', 0)), '-',
                    account_state(after.refs[0] if after.refs else None)]))
                scenarios.append('\t'.join([name, str(NOW), str(emulator.lt),
                                            shard.refs[0].boc().hex(), message.boc().hex(), '-']))
        finally:
            emulator.close()
        left.extend(rows)
        ...
        rust = subprocess.run([str(args.driver.resolve()), str(config_path), str(scenario_path),
                               str(version)], capture_output=True, text=True)
