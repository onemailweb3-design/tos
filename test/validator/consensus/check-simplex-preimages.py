...
def openssl_mldsa(key, message, signature, expect_valid):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        command = ['openssl', 'pkeyutl', '-verify', '-rawin', '-pubin', '-keyform', 'DER',
                   ...
                   '-pkeyopt', 'context-string:'+SIGN_CONTEXT]
...
def check(transcript_text, mldsa):
    ...
    require(set(vectors) == {'notarize', 'finalize', 'skip', 'proposal'},
    ...
    for name in ('notarize', 'finalize'):
        require(envelope(session, tag(name) + candidate_id) == vectors[name][1], name+' vote preimage')
    require(envelope(session, tag('skip') + struct.pack('<I', slot)) == vectors['skip'][1], 'skip vote preimage')
    ...
    return {'success': True, 'domains': sorted(vectors), 'slot': slot,
